...
SELECT COUNT(*) AS LowFat_Sales FROM blinkit_data WHERE Item_Fat_Content = 'Low Fat';

SELECT CAST(SUM(Total_Sales) AS DECIMAL(10, 0)) AS Estab2022_Sales FROM blinkit_data WHERE Outlet_Establishment_Year = 2022;
...
FROM blinkit_data GROUP BY Item_Fat_Content ORDER BY Total_Sales_Thousands DESC;
...
SELECT TOP 5 Item_Type,
  ...
  COUNT(*) AS No_Of_Items,
  CAST(AVG(Rating) AS DECIMAL(10, 2)) AS Avg_Rating
FROM blinkit_data GROUP BY Item_Type ORDER BY Total_Sales;
